...
export const useChatState = () => {
  ...
  const selectThread = async (conversationId: string) => {
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "대화방 선택에 실패했습니다.";
  ...
  /** 채팅이 아닌 다른 뷰(대시보드, 프라이싱 등)로 이동할 때 최근 대화 선택 하이라이트를 해제합니다. */
  ...
  const loadInitialData = async () => {
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "초기 데이터 로딩에 실패했습니다.";
  ...
  const sendMessage = async (
    ...
    try {
      ...
      await sendChatMessage(
        ...
        // onComplete: 스트림 완료 시
        async () => {
          console.log("[useChatState] onComplete 호출됨", {
            assistantMessageId,
          });
          isWaitingForResponse.value = false;
          ...
          if (messageIndex !== -1) {
            ...
            if (currentMessage) {
              ...
              console.warn("[useChatState] 메시지를 찾을 수 없음", {
                messageIndex,
              });
            }
          } else {
            console.warn("[useChatState] 메시지 인덱스를 찾을 수 없음", {
              assistantMessageId,
            });
          }
        },
      ...
      error.value =
        err instanceof Error ? err.message : "채팅 메시지 전송에 실패했습니다.";
      isWaitingForResponse.value = false;
  ...
  const deleteThread = async (conversationId: string) => {
    try {
      ...
      const wasActive = threads.value.some(
        (t) => t.conversationId === conversationId && t.active,
      );
